...
7. When a visitor selects `Show connector details`, the browser requests `/api/charger-details` for a station returned by a preceding search. The server fetches the individual EnBW station record and returns only EVSE ID, status, status update time, accessibility, plug type, power and cable information. For occupied/connected states, the browser uses the status timestamp to show an approximate connected duration. This does not claim that energy is actively flowing. Pricing and tariff objects are discarded.
