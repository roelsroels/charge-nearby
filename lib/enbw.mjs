...
function normaliseChargePoint(chargePoint, index) {
  const rawStatus = chargePoint?.state?.value || chargePoint?.status || "UNKNOWN";
  const plugs = Array.isArray(chargePoint?.connectors) ? chargePoint.connectors : [];
  return {
    id: chargePoint?.evseId ? String(chargePoint.evseId) : `charge-point-${index + 1}`,
    label: chargePoint?.chargePointLabel ? String(chargePoint.chargePointLabel) : null,
    status: String(rawStatus).toUpperCase(),
    updatedAt: isoTimestamp(chargePoint?.state?.updatedAt),
    accessible: chargePoint?.handicappedAccessible ?? null,
    plugs: plugs.map((plug) => ({
      type: plug?.chargePlugTypeGroup ? String(plug.chargePlugTypeGroup) : null,
      name: plug?.plugTypeName ? String(plug.plugTypeName) : null,
      powerKw: Math.max(0, Number(plug?.maxPowerInKw) || 0),
      cableAttached: plug?.cableAttached ?? null,
    })),
  };
}
...
export async function fetchStationDetails({
  ...
  const numericStationId = String(stationId || "");
  if (!/^\d{1,20}$/.test(numericStationId)) {
    throw new EnbwApiError("Invalid station ID", { status: 400 });
  }

  let response;
  try {
    response = await fetchImpl(`${API_BASE}/chargestations/${numericStationId}`, {
      headers: requestHeaders(apiKey),
      signal: AbortSignal.timeout(timeoutMs),
    });
  } catch (error) {
  ...
  if (response.status === 401 || response.status === 403) throw new EnbwAuthError();
  if (response.status === 404) throw new EnbwApiError("Station details are unavailable", { status: 404 });
  if (!response.ok) throw new EnbwApiError(`EnBW returned HTTP ${response.status}`);
  ...
  if (!data || typeof data !== "object" || Array.isArray(data)) {
    throw new EnbwApiError("EnBW returned an unexpected station response");
  }

  const chargePoints = Array.isArray(data.chargePoints) ? data.chargePoints : [];
  return {
    stationId: `enbw-${numericStationId}`,
    chargePoints: chargePoints
      .filter((chargePoint) => chargePoint && typeof chargePoint === "object")
      .map(normaliseChargePoint),
  };
}
